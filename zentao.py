...
def gen_a_testcase_row(testcase_dict):
    ...
    #
    case_title_list = case_title.split('|')
    if(len(case_title_list) < 2 ):
        print('Error! | not found', case_title)
    tmp_str = case_title_list[0]
    tmp_lst = tmp_str.split(' ')
    # print(tmp_lst)
    for str in tmp_lst:
        if str != '':
            case_module = case_module + '/' + str
    ...
    row = [case_module, case_title, case_precontion, case_step, case_expected_result, case_keyword, case_priority, case_apply_phase, case_type]
...
def gen_case_type(case_type):
    mapping = {1: '系统测试阶段', 2: '冒烟测试阶段', 3: '回归测试阶段'}
...
if __name__ == '__main__':
    ...
    xmind_file = 'docs/esim_7_smokecases.xmind'
    # xmind_file = 'docs/test1.xmind'
    zentao_csv_file = xmind_to_zentao_csv_file(xmind_file)
    print('step1: Conver the xmind file to a zentao csv file succssfully: %s', zentao_csv_file)
    # zentao_xlsx_file = CsvToXlsx(zentao_csv_file)
    # print('setp2: Conver the csv file to a zentao xlsx file succssfully: %s', zentao_xlsx_file)